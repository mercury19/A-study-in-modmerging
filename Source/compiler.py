...
def parse_int(value):
	if isinstance(value, list) or isinstance(value, tuple): return map(parse_int, value) ##If the value is in the list or tuple from the given class, apply the function parse_int() to all iterations of that value and return a list.
	...
	try:
		if value.is_static and value.module[5]: return value.__long__() & 0xffffffff ##If the value is static and the module for the value is indexed at 5, return the "& bit" of the value turned into a long and 0xffffffff
	except: pass
	return long(value) ##return the value converted into long format.
